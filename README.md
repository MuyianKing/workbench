...
- **首页工作台**：顶上是一行和这台机器有关的话 —— 一句按时间段的称呼（后面跟着设置里的程序名），加几条从本机数据里现取的实话（几个项目在跑、今天跑了几次、连着用了多少天）。下面九块可自由布局的卡片（活跃度、Coding 用量、系统状态、我的项目、快捷操作、快捷启动、命令、今日完成、AI 热点），卡片位置、栏数、栏宽、行的划分与高度随拖随存 —— 拖到另一张卡片的上 / 下缘就另起一行，拖到中间就并进那一行、几张并排平分栏宽；不看的可以在设置里关掉；左侧导航栏显示哪几页也能配。
