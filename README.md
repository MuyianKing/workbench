...
- 今天两页：**首页**（可自由布局的卡片画布）与**项目**（项目卡网格 + 分组筛选工具条）。
...
- 首页是一层工作区画布 + **七块可自由布局的卡片**：活跃度、Token 用量、系统状态、最近使用、
  快捷操作、快捷启动、命令。三栏结构，卡片可跨栏拖动、调高度、切固定 / 自适应，栏宽可拖
  （布局落在 `theme.json`）
...
- **Token 用量可以在多台机器之间同步**（默认关闭）：设置里填一个 git 仓库地址，每台机器各自把
  **自己那份用量快照**推到仓库的 `token-usage/<设备id>.json`，读的时候把各台机器的合起来算。
  ...
  同步只发往你给的那个仓库，凭据默认用系统里
  git 已经配好的那套；**登录过账号**（见下）就改用那个账号的 token 推送，私有仓库因此不必先手工配凭据。
...
- **外观配置也走同一个仓库，单独一个文件**（默认开启，可在设置里关掉）：设置里的开关打开时，
  ...
  换了机器或重装之后，打开设置 → 通用 → Token 同步：「从别的机器取外观」会列出仓库里有文件的其它机器
  ...
  本机才成立的项刻意不进这个文件：开机自启、全局快捷键、数据目录、Token 同步仓库地址各留各的
...
- 面板是「左侧菜单 + 右侧内容」两栏：菜单只有**外观**（主题、背景、顶部样式、首页布局）与
  **通用**（程序、快捷键、启动、数据目录、账号、Token 同步）两项，右侧各区独立滚动、菜单不跟着走
...
- 开机自启（默认开启，仅打包后生效）
- Token 同步（通用那一屏的最后一块）：填仓库地址（留空即不同步）、开关「同步外观配置」，
  以及「从别的机器取外观」—— 列出仓库里有文件的其它机器，逐台点「应用」把本机外观换成那一套；
  旁边那颗「同步一次」就地推拉一次，不必回首页（做法见上面的「外观配置也走同一个仓库…」）
...
- 登录的用途是**授权 Token 同步的私有仓库**：之后同步用这个账号的 token 推送，省掉「先在命令行里给
  git 配一次凭据」这一步。设置里可以关掉它退回系统 git 凭据 —— **这条退路要留着**：
...
src/renderer/src/        Vue 应用（组件 / Pinia store / 设计令牌）
  workbench/             window.workbench 的适配层（原 preload + 主进程逻辑的替代品）
    ...
    nvm.ts / nrm.ts / token.ts / system.ts / quick-launch.ts / auth.ts
  components/            公共与页面组件
...
**登录过账号的话这一步可以跳过**：同步会用账号的 token，不必再事先配 git 凭据。
...
- **Token 用量同步**（可选、默认关闭）：只有在设置里填了仓库地址，应用才会推 / 拉那个仓库
