...
- **技能**：管理一批自定义的 skill（一个文件夹 + 一份带 frontmatter 的 `SKILL.md`），名字与描述从清单里自动提取。技能库就住在笔记文件夹下的一个子目录里 —— 每次增删改自动记一版（版本就是 git 历史，可随时恢复），推到远端跟着笔记同步走；选一个项目就能把技能一键安装到它的 `.agents/skills/` 下（覆盖前会确认）。
