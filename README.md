...
- **笔记**：左边一棵文件夹 / 笔记混排的目录树（右键新建、改名、删除），右边 Vditor 写 markdown，边写边存。**只在本机**，不同步。
...
工作日志、笔记、本机设备标识。登录账号的 access_token **不进数据文件**，存在 Windows 凭据管理器里。
Token 同步只推模型名与计数，**不含对话内容**；工作日志与笔记则刻意只留在本机。
