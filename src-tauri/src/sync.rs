//! Token 快照的多机同步：把本机分片推进一个 git 仓库，再把别人的分片读回来。
//!
//! **为什么是 git**：仓库地址由用户在设置里给，凭据用它系统里 git 已经配好的那一套
//! （Credential Manager / SSH key），Workbench 自己不存任何 token。代价是本机要装 git。
//!
//! **冲突模型（整个方案能成立的地方）**：仓库里一台机器一个文件，每个文件只有一个写者，
//! 所以不存在需要人工合并的冲突，流程就是 pull → 写自己的文件 → commit → push。
//! 别人的分片我们从不修改，只在读取时求和（见 `shared/token-usage.ts` 的 combineShards）。
//!
//! 这一层仍然只做「调系统能力 + 读写文件」：分片的内容、合并语义、同步节流都在 TS 侧。
...
/// 写本机分片 → 提交 → 推送。返回 `{ changed, pushed, log }`（log 只用于排查）。
pub fn publish(repo: &str, device: &str, shard: &Value) -> Result<Value, String> {
    publish_at(&paths::token_shard_dir(), repo, device, shard)
}

/// 同上，克隆目录与分片目录由调用方给（单测用临时目录跑真实的 git 流程）。
fn publish_at(shard_dir: &Path, repo: &str, device: &str, shard: &Value) -> Result<Value, String> {
    let dir = shard_dir.parent().unwrap_or(shard_dir).to_path_buf();
    let (dir, branch) = ensure_clone(&dir, repo)?;
    let mut log: Vec<String> = Vec::new();

    let safe = safe_name(device);
    let file = shard_dir.join(format!("{safe}.json"));
    std::fs::create_dir_all(shard_dir).map_err(|err| format!("创建分片目录失败: {err}"))?;

    let text = serde_json::to_string_pretty(shard).map_err(|err| format!("序列化分片失败: {err}"))?;
    // 先写临时文件再改名：读到半个 JSON 的可能是 git 自己，也可能是同步目录上的杀毒软件
    let tmp = file.with_extension("json.tmp");
    std::fs::write(&tmp, &text).map_err(|err| format!("写入分片失败: {err}"))?;
    std::fs::rename(&tmp, &file).map_err(|err| format!("替换分片失败: {err}"))?;

    let relative = format!("devices/{safe}.json");
    run_git(&["add", "--", &relative], Some(&dir), GIT_TIMEOUT)?;

    // 有没有东西要提交交给 git 自己判断（内容没变时自动同步每分钟都会走到这里，
    // 不能让仓库里堆一串「什么都没改」的提交）。`diff --cached --quiet` 退出码 1 = 有暂存改动。
    let changed = !run_git_quiet(&["diff", "--cached", "--quiet", "--", &relative], &dir);
    if changed {
        let message = format!("token-sync:{safe}");
...
/// 读同步目录里所有机器的分片（原始 JSON，收敛与合并由渲染层负责）。
///
/// **只读属于当前配置那个仓库的克隆**：克隆存在但 origin 不是 `repo` 时返回空表。
/// 换了地址之后重新克隆可能还没成功（没网 / 新地址写错 / 旧克隆被占用），
/// 而克隆的工作区里还留着**老仓库**的别人的分片 —— 不核对的话那些文件会被当成最新的读进来，
/// 界面上就是一份「看着正常、其实来自另一个仓库」的数字。
///
/// 不做别的过滤：本机那一份也在返回里，由渲染层按 device 排除
/// （它手上有内存里的最新版本，用仓库里上次推送的副本会把本机重复计一遍）。
pub fn read_shards(repo: &str) -> Result<Vec<Value>, String> {
    read_shards_for(&paths::token_sync_dir(), repo)
}

/// 同上的实体：克隆目录可注入（单测用临时目录跑真实 git）
fn read_shards_for(dir: &Path, repo: &str) -> Result<Vec<Value>, String> {
    if !dir.join(".git").is_dir() || !clone_matches(dir, repo) {
        return Ok(Vec::new());
    }
    read_shards_at(&dir.join(paths::SHARD_DIR))
}

fn read_shards_at(dir: &Path) -> Result<Vec<Value>, String> {
    ...
    for entry in entries.flatten() {
        ...
        // 单个文件坏了（手工改过 / 半截写入）就跳过它：一个坏分片不能把整块面板拖垮
...
fn run(args: &[&str], cwd: Option<&Path>, timeout: Duration) -> Result<proc::Outcome, String> {
    ...
    proc::run_direct("git", &owned, timeout, cwd, &GIT_ENVS).map_err(|err| {
...
mod tests {
    ...
    /// 没有克隆目录时读分片是空列表而不是报错（还没同步过的机器就是这个状态）
    #[test]
    fn reading_shards_without_a_clone_is_empty() {
        let missing = std::env::temp_dir().join("wb-sync-does-not-exist").join("devices");
        assert!(read_shards_at(&missing).unwrap().is_empty());
    }
    ...
    fn reading_shards_of_another_repo_is_empty() {
        ...
        let shard = json!({ "version": 4, "device": "dev-z", "name": "Z", "updatedAt": 1, "sources": {} });

        // 克隆里现在装的是 A 的分片
        publish_at(&clone.join("devices"), &repo_a, "dev-z", &shard).unwrap();

        // 地址还是 A：读得到；换成 B：一个都不返回（老仓库的分片不能当成最新的展示出来）
        assert_eq!(read_shards_for(&clone, &repo_a).unwrap().len(), 1);
        assert!(read_shards_for(&clone, &repo_b).unwrap().is_empty());
    ...
    fn syncs_two_machines_through_a_real_git_remote() {
        ...
        let shard_a = json!({
            "version": 4, "device": "dev-a", "name": "A", "updatedAt": 1,
            "sources": { "zcode": { "days": {} } }
        });
        let shard_b = json!({
            "version": 4, "device": "dev-b", "name": "B", "updatedAt": 2,
            "sources": { "zcode": { "days": {} } }
        });

        let first = publish_at(&clone_a.join("devices"), &repo, "dev-a", &shard_a).unwrap();
        ...
        let second = publish_at(&clone_b.join("devices"), &repo, "dev-b", &shard_b).unwrap();
        assert_eq!(second["changed"], json!(true));

        let read = read_shards_at(&clone_b.join("devices")).unwrap();
        let mut devices: Vec<String> = read
            .iter()
        ...
        // 内容没变就不该产生提交：自动同步十分钟一轮，否则仓库里全是空提交
        let again = publish_at(&clone_a.join("devices"), &repo, "dev-a", &shard_a).unwrap();
    ...
    fn switching_the_repo_reclones_and_drops_the_old_shards() {
        ...
        let repo_a = remote_a.to_string_lossy().into_owned();
        let repo_b = remote_b.to_string_lossy().into_owned();
        let devices = clone.join("devices");
        let shard = |device: &str| {
            json!({ "version": 4, "device": device, "name": device, "updatedAt": 1, "sources": {} })
        };

        // 先在 A 上同步，并让另一台机器（dev-z）的分片也进了这个克隆
        publish_at(&devices, &repo_a, "dev-z", &shard("dev-z")).unwrap();
        assert_eq!(read_shards_at(&devices).unwrap().len(), 1);

        // 换到 B
        publish_at(&devices, &repo_b, "dev-a", &shard("dev-a")).unwrap();

        let read = read_shards_at(&devices).unwrap();
        let found: Vec<String> = read
            .iter()
            .filter_map(|item| item.get("device").and_then(Value::as_str).map(str::to_string))
            .collect();
        assert_eq!(found, vec!["dev-a".to_string()], "换了仓库之后不能再看到老仓库的分片");
