...
- 技能库是**它自己的一个目录**（设置里的 `skillDir`，技能页那颗「选择技能文件夹」挑的，
  记在本机数据文件里）：**与笔记文件夹互不相干** —— 两边各自挑各自的，可以正好是同一处、同一仓库，
  也可以不是。所以**不要再把技能与笔记绑在一起**（曾有过「技能库住在笔记仓库某一层」的设计）。
- 「它在哪个仓库里」不用配、也不许猜：从技能库目录开始看有没有 `.git`，没有就往上找最近的
  （`skills.rs` 的 `state`，8 层为限）。它给出「仓库根 + 库在仓库里的相对路径」，
  各条技能通道拿的正是这两样；**库自己就是仓库根**时那个相对路径是空串（专门的技能仓库就是这种，
  `rel_of` 必须收空串）。与笔记那边**故意不同**：笔记同步只认「这个文件夹自己有 `.git`」，
  因为 `commit_notes` 是 `git add -A` 不带 pathspec；技能这边的提交**带 pathspec**（只动技能库那一条路径），
  所以往上找是安全的 —— 两者别互相「对齐」。
- 增删改只在本机提交（`skills.rs` 的 `commit`），推上去走 `skills.rs` 的 `sync`：
  **先提交技能库那一层**（`add -A -- <库>`），再 `pull --rebase --autostash`、`push` ——
  拉与推是整条分支（git 没有「半个分支」），但**提交范围只限技能库**：同一个仓库里别的东西一概不捎带。
  凭据走 `set_git_auth`（与笔记同步同一套：账号 token 或系统 git 凭据）。
- **提交身份**：`commit` 与 `sync` 都要先 `ensure_commit_identity`（用户没配过全局 git 身份时的兜底）——
  这条路不经过笔记同步，不能指望那边顺手写进去。
